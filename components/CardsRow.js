...
const CardComponent = ({ title, description, image }) => (
    <div className="col-lg-6 d-flex mb-4">
      <div className={`card flex-row flex-grow-1 ${styles.customCard}`}>  
        ...
        <Image src={image} className={`card-img-right ${styles.image}`} alt={title} width={50} height={50} layout="responsive" />
    ...
    <div className={`container-fluid ${styles.cardContainer}`}>
      <div className="row">
        {cardsContent.map((card, index) => (
          <CardComponent key={index} title={card.title} description={card.description} image={card.image} />
        ))}
      </div>
    </div>
  );
